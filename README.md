...
### Вывод
Таким образом, лидером как по размеру исполняемого файла, так и по размеру ассемблерного кода стала собственная программа. Однако по времени её результат не может сравниться с результатами программ, которые компилировались с опциями оптимизации по скорости.